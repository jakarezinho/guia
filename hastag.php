...
<body>
  ...
  <?php if ($nbPages > 0) : ?>
    ...
    <ul class="list-inline"><?php $pages->hastag_links('hastag.php', $hastag, $hastag) ?></ul>

    <div class="boton_map">
      <p class="text-center"> <i class="material-icons">map</i>
        <?php if ($refer) : ?>
          <a href="mapa_hastag.php?hastag=<?= $hastag; ?>&refer=<?= $refer; ?>"> ver no mapa» </a>
      </p>
    <?php else : ?>
  ...
  ?>
    ...
    <div id="paginate" class="nav">
      ...
      if ($Cpage < $nbPages && $refer) {
        echo '<a class="pag-next" href="hastag.php?p=' . ($Cpage + 1) . '&hastag=' . $hastag . '&refer=' . $refer . '"></a>';
      } elseif ($Cpage < $nbPages) {
        echo '<a class="pag-next" href="hastag.php?p=' . ($Cpage + 1) . '&hastag=' . $hastag . '"></a>';
      } ?>
    </div>
