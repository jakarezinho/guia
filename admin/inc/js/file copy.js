  ...
  if (document.body.contains(document.getElementById('lat')) && document.body.contains(document.getElementById('lng'))) {
      ...
      if (latDepart.value.length == 0 && lngDepart.value.length == 0 && localisation.value.length == 0) {
      ...
      } else if (latDepart.value.length > 0 && lngDepart.value.length > 0 && localisation.value == '') {
          map = L.map('mapa').setView([latDepart.value, lngDepart.value], 14);
          initMapa().addTo(map)
          r.addTo(map);
          let marker_gps = L.marker([latDepart.value, lngDepart.value], {
              draggable: true
          })
          r.addLayer(marker_gps);
          console.log('sem morada' + latDepart.value)
          reversse(latDepart.value, lngDepart.value)
          console.log('sem morada')
          ...
          console.log('com tudo' + localisation)
      }
